...
fun Body(modifier: Modifier) {
    ...
    Column(modifier = modifier) {
        ...
        Email(email, { email = it }, Modifier.fillMaxWidth())
        Spacer(modifier = Modifier.size(4.dp))
        Password(password, { password = it }, Modifier.fillMaxWidth())
...
fun Password(password: String, onTextChange: (String) -> Unit, modifier: Modifier) {
    TextField(value = password , onValueChange = { onTextChange(it) }, modifier = modifier)
}

@Composable
fun Email( email: String, onTextChange:(String) -> Unit, modifier: Modifier) {
    TextField(value = email , onValueChange = { onTextChange(it) }, modifier = modifier)
}
